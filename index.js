...
function checkRatio(invites) {
	...
	maleFound = false,
	femaleFound = false;
	for (var i=0, invitesLength = invites.length; i < invitesLength; i++) {
		if (invites[i].first_name.indexOf(' ') > 0) {
			invites[i].first_name = invites[i].first_name.substring(0, invites[i].first_name.indexOf(' '));
		}
		invites[i].first_name = removeDiacritics(invites[i].first_name.toUpperCase());
		maleFound = false;
		femaleFound = false;
		correspondingMaleTable = maleNames[invites[i].first_name.substring(0,1)];
		correspondingFemaleTable = femaleNames[invites[i].first_name.substring(0,1)];
		if (correspondingMaleTable) {
			for (var j=0, malesLength = correspondingMaleTable.length; j < malesLength; j++) {
				if (invites[i].first_name === removeDiacritics(correspondingMaleTable[j].toUpperCase())) {
					maleCounter++;
					maleFound = true;
					break;
				}
			...
			if (!maleFound) {
				for (var k=0, femalesLength = correspondingFemaleTable.length; k < femalesLength; k++) {
					if (invites[i].first_name === removeDiacritics(correspondingFemaleTable[k].toUpperCase())) {
						femaleCounter++;
						femaleFound = true;
						break;
					}
